...
from model.pfa_yolov5.utils import TryExcept
from model.pfa_yolov5.utils.dataloaders import exif_transpose, letterbox
...
from model.pfa_yolov5.utils.plots import Annotator, colors, save_one_box
from model.pfa_yolov5.utils.torch_utils import copy_attr, smart_inference_mode
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(
        ...
        if pt:  # PyTorch
            ...
            # LOGGER.info(f"Loading {w} for TensorRT inference...")
            # import tensorrt as trt  # https://developer.nvidia.com/nvidia-tensorrt-download
            # check_version(trt.__version__, "10.0.0", hard=True)  # require tensorrt>=10.0.0
            ...
            from model.pfa_yolov5.utils.triton import TritonRemoteModel
    ...
    def forward(self, im, augment=False, visualize=False):
        ...
        # Ensure input batch size is within profile range
            # profile_batch_size = self.model.get_tensor_profile_shape("images", 0)[2][0]  # Max batch size from profile
            # if im.shape[0] > profile_batch_size:
            #     print(f"Input batch size {im.shape[0]} exceeds max supported batch size {profile_batch_size}")
            #     im = im[:profile_batch_size]  # Truncate batch size if necessary
            
            # if self.dynamic and im.shape != self.bindings["images"].shape:
            #     # Update input shape for dynamic input
            #     self.context.set_input_shape("images", im.shape)
            #     self.bindings["images"] = self.bindings["images"]._replace(
            #         shape=im.shape,
            #         data=torch.from_numpy(np.empty(im.shape, dtype=self.bindings["images"].dtype)).to(im.device),
            #         ptr=int(im.data_ptr())
            #     )
            #     # Resize output bindings
            #     for name in self.output_names:
            #         new_shape = tuple(self.context.get_tensor_shape(name))
            #         self.bindings[name] = self.bindings[name]._replace(
            #             shape=new_shape,
            #             data=torch.from_numpy(np.empty(new_shape, dtype=self.bindings[name].dtype)).to(im.device),
            #             ptr=int(self.bindings[name].data.data_ptr())
            #         )
            #         self.binding_addrs[name] = int(self.bindings[name].data.data_ptr())
            
            # s = self.bindings["images"].shape
            # assert im.shape == s, f"Input size {im.shape} not equal to model size {s}"
            # self.bindings["images"].data[:] = im  # Copy input data
            # self.binding_addrs["images"] = int(im.data_ptr())
            # self.context.execute_v2(list(self.binding_addrs.values()))
            # y = [self.bindings[x].data for x in sorted(self.output_names)]
            if self.dynamic and im.shape != self.bindings["images"].shape:
                self.is_trt10 = True # allow for TensorRT 10.0+ dynamic shape handling
                if self.is_trt10:
                    self.context.set_input_shape("images", im.shape)
    ...
    def _model_type(p="path/to/model.pt"):
        ...
        from model.pfa_yolov5.utils.downloads import is_url
