...
from model.pfa_yolov5.utils.dataloaders import LoadImages
...
from model.pfa_yolov5.utils.torch_utils import select_device, smart_inference_mode
    ...
    if bits < 32:
        if MACOS:  # quantization only supported on macOS
            with warnings.catch_warnings():
                ...
                ct_model = ct.models.neural_network.quantization_model.pfa_yolov5.utils.quantize_weights(
                    ct_model, bits, mode
                )
        else:
...
def parse_opt():
    ...
    parser.add_argument(
        "--data", type=str, default=ROOT / "yaml/armor.yaml", help="dataset.yaml path"
    )
    parser.add_argument(
        "--weights",
        nargs="+",
        type=str,
        default=ROOT / "models/armor.onnx",
    ...
    parser.add_argument(
        "--workspace", type=int, default=6, help="TensorRT: workspace size (GB)"
    )
